...
class LaplacianBuilder(nn.Module):
    ...
    def scalar_normalise(self, diag, tril, row, col):
        ...
        diag_sqrt_inv = diag_sqrt_inv.view(-1, 1,
                                           1) if tril.dim() > 2 else diag_sqrt_inv.view(
            -1, d)
        ...
        diag_sqrt_inv = diag_sqrt_inv.view(-1, 1,
                                           1) if diag.dim() > 2 else diag_sqrt_inv.view(
            -1, d)
    ...
    def append_fixed_maps(self, size, diag_indices, diag_maps, tril_indices, tril_maps):
        ...
        if self.normalised:
            fixed_diag, fixed_non_diag = self.scalar_normalise(fixed_diag,
                                                               fixed_non_diag, tril_row,
                                                               tril_col)
        fixed_diag, fixed_non_diag = fixed_diag.view(-1), fixed_non_diag.view(-1)
        # Combine the learnable and fixed parts.
        tril_indices, tril_maps = lap.mergesp(self.fixed_tril_indices, fixed_non_diag,
                                              tril_indices, tril_maps)
        diag_indices, diag_maps = lap.mergesp(self.fixed_diag_indices, fixed_diag,
...
class DiagLaplacianBuilder(LaplacianBuilder):
    ...
    def normalise(self, diag, tril, row, col):
        if self.normalised:
            ...
            deg_sqrt_inv = (self.deg + 1).pow(-0.5) if self.augmented else self.deg.pow(
                -0.5)
    ...
    def forward(self, maps):
        ...
        non_diag_indices, non_diag_values = lap.mergesp(tril_indices, tril_maps,
                                                        triu_indices, tril_maps)

        # Merge diagonal and non-diagonal
        edge_index, weights = lap.mergesp(non_diag_indices, non_diag_values,
                                          diag_indices, diag_maps)
...
class NormConnectionLaplacianBuilder(LaplacianBuilder):
    ...
    def __init__(self, size, edge_index, d, add_hp=False, add_lp=False, orth_map=None,
                 augmented=True):
        super(NormConnectionLaplacianBuilder, self).__init__(
            size, edge_index, d, add_hp=add_hp, add_lp=add_lp, normalised=True,
            augmented=augmented)
    ...
    def create_with_new_edge_index(self, edge_index):
        assert edge_index.max() <= self.size
        new_builder = self.__class__(
            self.size, edge_index, self.d, add_hp=self.add_hp, add_lp=self.add_lp,
            augmented=self.augmented,
    ...
    def forward(self, map_params, edge_weights=None):
        ...
        # Normalise the entries if the normalised Laplacian is used.
        diag_maps, tril_maps = self.scalar_normalise(diag_maps, tril_maps, tril_row,
                                                     tril_col)
    ...
    def normalise(self, diag_maps, non_diag_maps, tril_row, tril_col):
        if self.normalised:
            ...
            if self.training:
                ...
                eps = torch.FloatTensor(self.d).uniform_(-0.001, 0.001).to(
                    device=self.device)
            else:
                eps = torch.zeros(self.d, device=self.device)

            to_be_inv_diag_maps = diag_maps + torch.diag(1. + eps).unsqueeze(
                0) if self.augmented else diag_maps
            ...
            non_diag_maps = (left_norm @ non_diag_maps @ right_norm).clamp(min=-1,
                                                                           max=1)
