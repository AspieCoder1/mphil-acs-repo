...
class RGCNNodeClassifier(L.LightningModule):
    def __init__(
        ...
        super().__init__()
        self.model = RGCN(
            hidden_channels, out_channels, num_nodes, num_relations, target=target
        )
    ...
    def common_step(
        self, batch: Batch, mask: torch.Tensor
    ...
    def validation_step(self, batch: Batch, batch_idx: int) -> STEP_OUTPUT:
        ...
        self.log(
            "valid/accuracy", self.test_acc, prog_bar=True, on_step=False, on_epoch=True
        )
        self.log(
            "valid/loss",
            loss,
    ...
    def test_step(self, batch: Batch, batch_idx: int) -> STEP_OUTPUT:
        ...
        self.log(
            "test/accuracy", self.test_acc, prog_bar=True, on_step=False, on_epoch=True
        )
        self.log(
            "test/f1-score", self.test_f1, prog_bar=False, on_step=False, on_epoch=True
        )
        self.log(
            "test/auroc", self.test_auroc, prog_bar=False, on_step=False, on_epoch=True
        )
        self.log(
            "test/loss",
            loss,
