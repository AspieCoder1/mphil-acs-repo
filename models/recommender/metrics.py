...
class LinkPredMetric(torchmetrics.Metric):
    ...
    def __init__(self, k: int) -> None:
        ...
        if k <= 0:
            raise ValueError(
                f"'k' needs to be a positive integer in "
    ...
    def update(
        ...
        if pred_index_mat.size(1) != self.k:
            raise ValueError(
                f"Expected 'pred_index_mat' to hold {self.k} "
        ...
        max_index = (
            max(  # type: ignore
                ...
                edge_label_index[1].max() if edge_label_index[1].numel() > 0 else 0,
            )
            + 1
        )
        arange = torch.arange(
...
class LinkPredMAP(LinkPredMetric):
    ...
    def _compute(self, pred_isin_mat: Tensor, y_count: Tensor) -> Tensor:
        cum_precision = torch.cumsum(pred_isin_mat, dim=1) / torch.arange(
            1, self.k + 1, device=y_count.device
        )
        return (cum_precision * pred_isin_mat).sum(dim=-1) / y_count.clamp(
            min=1e-7, max=self.k
        )
